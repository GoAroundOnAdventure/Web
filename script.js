...
if (notifyForm) {
    notifyForm.addEventListener('submit', async (e) => {
        ...
        if (email) {
            ...
            const isValidDomain = allowedDomains.some(domain => lowerEmail.endsWith(`@${domain}`));
            
            // 1. Domain Validation
            if (!isValidDomain) {
                showToast(`Supported providers: ${allowedDomains.join(', ')}`, true);
                return;
            }
            ...
            try {
                ...
                notifyForm.reset();
                if (domainShortcuts) domainShortcuts.style.display = 'none';
            } catch (error) {
...
// 6. Domain Shortcut Pills Logic
const emailInputForSuggestions = document.querySelector('.notify-form input');
const domainShortcuts = document.querySelector('.domain-shortcuts');
const domainPills = document.querySelectorAll('.domain-shortcuts span');

if (emailInputForSuggestions && domainShortcuts) {
    // Show/hide pills based on input
    emailInputForSuggestions.addEventListener('input', (e) => {
        const val = e.target.value.trim();
        if (val) {
            domainShortcuts.style.display = 'flex';
        } else {
            domainShortcuts.style.display = 'none';
        }
    });
    ...
    domainPills.forEach(pill => {
        pill.addEventListener('click', () => {
            const domain = pill.textContent.trim();
            let currentVal = emailInputForSuggestions.value.trim();
            
            // If the user already has an '@', use what's before it
            if (currentVal.includes('@')) {
                currentVal = currentVal.split('@')[0];
            }
            
            if (currentVal) {
                emailInputForSuggestions.value = currentVal + domain;
                emailInputForSuggestions.focus();
            } else {
                showToast("Type your username first!", true);
            }
        });
    });
}
